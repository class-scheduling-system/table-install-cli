...
-- 创建触发器，当冲突标记为已解决时自动记录解决时间
DELIMITER //
CREATE TRIGGER `trg_conflict_resolved`
    BEFORE UPDATE ON `cs_scheduling_conflict`
    FOR EACH ROW
BEGIN
    IF NEW.resolution_status = 1 AND OLD.resolution_status != 1 THEN
