...
(
    ...
    `is_graduated` BOOLEAN     NOT NULL DEFAULT FALSE COMMENT '是否毕业 0:未毕业 1:已毕业',
...
CREATE INDEX `idx_student_grade_uuid` ON `cs_student` (`grade_uuid`) COMMENT '年级索引';
CREATE INDEX `idx_student_is_graduated` ON `cs_student` (`is_graduated`) COMMENT '毕业状态索引';
...
ALTER TABLE `cs_student`
    ...
    ADD CONSTRAINT `fk_cs_student_cs_grade`
