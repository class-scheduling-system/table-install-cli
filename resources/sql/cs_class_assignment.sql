...
(
    ...
    `building_uuid`                   CHAR(32)          NOT NULL COMMENT '教学楼主键',
    `classroom_uuid`                  CHAR(32)          NOT NULL COMMENT '教室主键',
    `teaching_class_uuid`             CHAR(32)          NOT NULL COMMENT '教学班主键',
    `course_ownership`                VARCHAR(32)       NOT NULL COMMENT '课程归属',
    `credit_hour_type`                CHAR(32)          NOT NULL COMMENT '学时类型',
    `teaching_hours`                  DECIMAL(10, 2)    NOT NULL DEFAULT 0 COMMENT '教学学时（指教师实际授课的学时）',
    `scheduled_hours`                 DECIMAL(10, 2)    NOT NULL DEFAULT 0 COMMENT '排课学时（指课程安排的学时）',
    `total_hours`                     DECIMAL(10, 2)    NOT NULL DEFAULT 0 COMMENT '总需学时',
...
CREATE INDEX `idx_class_assignment_teaching_class` ON `cs_class_assignment` (`teaching_class_uuid`) COMMENT '教学班索引';
...
ALTER TABLE `cs_class_assignment`
    ...
    ADD CONSTRAINT `fk_class_assignment_teacher_uuid`
        FOREIGN KEY (`teacher_uuid`) REFERENCES `cs_teacher` (`teacher_uuid`)
            ON DELETE RESTRICT ON UPDATE CASCADE,
    ADD CONSTRAINT `fk_class_assignment_campus_uuid`
        FOREIGN KEY (`campus_uuid`) REFERENCES `cs_campus` (`campus_uuid`)
            ON DELETE RESTRICT ON UPDATE CASCADE,
    ADD CONSTRAINT `fk_class_assignment_building_uuid`
    ...
    ADD CONSTRAINT `fk_class_assignment_classroom_uuid`
        FOREIGN KEY (`classroom_uuid`) REFERENCES `cs_classroom` (`classroom_uuid`)
            ON DELETE RESTRICT ON UPDATE CASCADE,
    ADD CONSTRAINT `fk_class_assignment_teaching_class_uuid`
        FOREIGN KEY (`teaching_class_uuid`) REFERENCES `cs_teaching_class` (`teaching_class_uuid`)
            ON DELETE RESTRICT ON UPDATE CASCADE,
